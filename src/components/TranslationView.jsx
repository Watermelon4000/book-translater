    ...
    const handleStart = async () => {
        ...
        try {
            for (const chapter of book.spine) {
                ...
                // Translate content
                const { translatedHtml, stats } = await mgr.processChapter(chapter.href, (pct) => {
                    // Optional: Update granular progress for this chapter if UI supports it
                });

                if (stats.failedChunks > 0) {
                    totalErrors += stats.failedChunks;
                    setErrorCount(prev => prev + stats.failedChunks);
                    console.warn(`Chapter ${chapter.href} had ${stats.failedChunks} failed chunks.`);
                }
        ...
        <div className="max-w-4xl mx-auto space-y-8 animate-fade-in pb-20">
            ...
            {errorCount > 0 && (
                <div className="bg-red-50 border border-red-200 rounded-lg p-4 flex items-center gap-3 text-red-800 animate-fade-in">
                    ...
                    <div>
                        <p className="font-bold">Warning: {errorCount} text segments failed to translate.</p>
                        <p className="text-sm">This usually happens due to API rate limits or invalid keys. The result will contain some original English text.</p>
                    </div>
                </div>
            )}
